...
La barre n'affiche que 4 onglets — Accueil, Progression, Scanner, Profil — plus un bouton **+** flottant au centre qui n'est pas un onglet : `coach`, `workout` et `meals` sont déclarés comme `Tabs.Screen` avec `href: null`, ce qui les retire de la barre sans les retirer du routeur (`/coach`, `/workout`, `/meals` restent navigables en poussant l'écran par-dessus les tabs). Le bouton **+** est un `Pressable` absolument positionné à côté du `<Tabs>` (pas un `tabBarButton`), un cercle de 56px `colors.accent` qui dépasse d'environ 16px au-dessus du haut de la barre (`fabBottom = tabBarBottom + TAB_BAR_HEIGHT - (FAB_SIZE - FAB_PROTRUSION)`), avec une icône `Plus` en `colors.background` (même convention que le variant primaire de `Button.tsx`, texte foncé sur fond clair) et une ombre portée. Les deux onglets adjacents au bouton (`progression` à sa gauche, `scanner` à sa droite) reçoivent chacun une marge (`CENTER_GAP_HALF = 32`) pour garantir qu'aucun libellé ne passe sous le bouton, quelle que soit la largeur d'écran.

Le bouton ouvre `components/ui/NavigationSheet.tsx`, une feuille modale "faite maison" avec `Modal` + `Animated` + `PanResponder` (pas de dépendance externe — même convention que `ChoiceModal.tsx`/`TimePickerModal.tsx`) : elle glisse depuis le bas (`translateY` animé, fond `colors.surface`, coins hauts arrondis à 24px, poignée grise centrée), avec un fond assombri (`rgba(0,0,0,0.6)`) qui se ferme au tap comme la feuille elle-même au balayage vers le bas (relâchement au-delà de 100px ou à vitesse suffisante). La grille de navigation est en 2 colonnes fixes — volontairement chunkée en paires (`GRID_ROWS`) plutôt que laissée à `flexWrap`, qui aurait fait tenir 3 cartes de 110px par ligne sur un écran large. Coach IA, Mes séances, Mes repas et Notifications sont cliquables et ferment la feuille avant de naviguer ; Groupes, Widgets, Recettes et Badges sont grisés avec un badge "Bientôt" et affichent une alerte "Bientôt disponible" au tap plutôt que de naviguer.
