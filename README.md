...
Toutes les animations utilisent l'API `Animated` de React Native (`useRef(new Animated.Value())`, `Animated.timing`/`Animated.spring`/`Animated.sequence`/`Animated.loop`), avec `useNativeDriver: true` partout où c'est possible (opacité, transformations) — aucune animation supérieure à 400ms, aucune ne bloque une interaction. `react-native-reanimated` a été essayé en premier, mais sa v4 dépend nativement de `react-native-worklets`, un module natif absent d'Expo Go (nécessite un development build) : dans Expo Go, le charger plantait l'app au démarrage (`Exception in HostFunction`) même une fois les deux paquets alignés sur les versions bundlées par le SDK. Les deux ont donc été entièrement retirés (voir `.npmrc` ci-dessous) et remplacés par `Animated`, en gardant exactement les mêmes durées/courbes/effets. Une seule exception à `useNativeDriver: true`, documentée en commentaire dans le code : le compteur de l'écran de résultat (ne pilote aucun style, juste un `addListener` qui alimente un `useState`, donc le driver natif n'apporte rien). La barre de progression, elle aussi native-driver, n'anime pas `width` (propriété de layout, jamais éligible) mais `transform: [{ scaleX }]` sur un remplissage à 100% ancré à gauche via `transformOrigin: 'left'` — même rendu visuel (la barre grandit de gauche à droite), entièrement sur le thread natif. Les transitions de couleur (fond/texte des cartes et du bouton "Continuer" à la sélection) ne sont, elles, pas animées du tout — `backgroundColor`/`color` ne passent pas non plus par le driver natif, et les animer sur le thread JS dégraderait les perfs pour un gain visuel marginal ; elles basculent donc instantanément avec la prop, pendant que le pulse d'échelle (transform, natif) reste animé. Retours haptiques via `expo-haptics` (compatible Expo Go, léger à la sélection, moyen sur "Continuer", succès sur les écrans de respiration/résultat/fin).

**`.npmrc`** (`omit=optional`) — `react-native-reanimated` reste listé comme peer dependency *optionnelle* de `expo-router` (pour son intégration facultative avec `react-native-gesture-handler`, elle-même protégée par un `try/catch` — voir son code). Sans ce réglage, `npm install` réinstallait quand même les deux paquets malgré leur suppression de `package.json`, à cause de la résolution automatique des peer dependencies optionnelles de npm 7+. `omit=optional` est le mécanisme standard pour les exclure explicitement du dossier `node_modules` ; vérifié après coup que `react-native-reanimated`/`react-native-worklets` n'apparaissent plus du tout dans le bundle exporté (`npx expo export`).
...
**Envoi d'images au coach.** Bouton trombone (`Paperclip`, `components/coach/ChatInput.tsx`) ouvrant `expo-image-picker`, aperçu 60px au-dessus du champ de saisie avant envoi. `moderateCoachImage` (`lib/coach.ts`) fait un appel `FAST_MODEL` dédié et minimal (60 tokens max, pas d'outils) avant tout upload : `{"approuve": true}` ou `{"approuve": false, "raison": "..."}` sur une image compressée (1024px, qualité 0.6, `compressImage` de `lib/foodScanner.ts` — accepte maintenant une qualité en 4ᵉ paramètre optionnel, 0.5 par défaut pour ne rien changer aux appelants existants). L'appel utilise un préremplissage (`{ role: 'assistant', content: '{' }`) qui force la réponse à commencer par une accolade — le prompt demandait déjà "JSON uniquement, sans texte d'introduction" mais le modèle l'ignorait parfois, ce qui produisait un texte du type « Bien sûr, voici : {...} » et faisait planter un `JSON.parse` brut. La réponse brute est journalisée avec `console.log` avant tout parsing pour pouvoir diagnostiquer un futur cas non prévu. `moderateCoachImage` retourne `'approved' | 'rejected' | 'check_failed'` plutôt qu'un simple booléen, pour distinguer une image explicitement refusée par le modèle (`hooks/useCoachMessages.ts` affiche « Cette image ne peut pas être analysée. Envoie une photo en lien avec ton objectif. ») d'un échec de la vérification elle-même — réseau, HTTP non-2xx, ou réponse toujours illisible malgré le nettoyage (« La vérification de l'image a échoué. Réessaie. ») — les deux cas bloquent l'envoi, sans jamais planter. Une image refusée ou dont la vérification échoue n'est ni compressée-uploadée ni transmise au coach. Une image acceptée est téléversée dans le bucket privé `coach-images` (`{user_id}/{timestamp}.jpg`, chemin toujours unique donc pas de risque de collision de cache), l'URL signée obtenue juste après l'upload reçoit le même correctif anti-cache que l'avatar de profil (`&t=${Date.now()}` ajouté à l'URL), puis l'image (base64) est envoyée dans le même message Anthropic que le texte — `sendMessage` accepte un `image` optionnel et le rattache au dernier message du tableau plutôt qu'à un message dédié. La bulle utilisateur affiche l'image à 200×200 (`components/coach/MessageBubble.tsx`), au-dessus du texte s'il y en a un.

**Nettoyage JSON partagé.** `extractJsonObject` (`lib/anthropic.ts`) enchaîne `stripJsonFences` (retire un bloc ```` ```json ```` englobant) puis extrait le premier objet `{...}` via `/\{[\s\S]*\}/` — malgré la consigne "JSON uniquement" répétée dans chaque prompt de cette app, le modèle ajoute parfois une phrase avant ou après, ce qui fait échouer un `JSON.parse` brut avec `Unexpected character` sur un JSON par ailleurs valide. Tous les appels Anthropic qui attendent du JSON en retour (coach — modération d'image, scanner de repas, recettes, analyse de progression, skincare) passent par cette même fonction plutôt que par `stripJsonFences` seul ou un nettoyage réinventé localement.
...
`(tabs)/progression.tsx` empile, dans l'ordre : carte de poids actuel (+ modale de saisie), carte "Objectif estimé", graphique d'évolution, grille de 4 statistiques, IMC, hydratation, moyenne calorique, énergie hebdomadaire, dépenses énergétiques, série de régularité (30 jours), badges obtenus (masquée si aucun badge) et photos de progression.
...
Le graphique (`components/progression/WeightChart.tsx`) est tracé à la main avec `react-native-svg` (même bibliothèque que `CalorieRing` sur le dashboard) plutôt qu'avec une lib de charts : ligne + points + dégradé sous la courbe + ligne pointillée au niveau de l'objectif, avec un domaine vertical qui s'étend toujours pour inclure l'objectif même s'il est loin des mesures récentes. En dessous de deux mesures sur la période sélectionnée, l'écran affiche un état vide plutôt qu'un graphique vide ou une erreur. Cinq périodes (3j/7j/14j/30j/90j, `PERIOD_OPTIONS` dans `constants/progression.ts`) filtrent `weight_logs` ; la densité des étiquettes de l'axe horizontal s'adapte à la période plutôt que de toujours afficher premier/milieu/dernier point (`pickAxisLabelCount` — 7 étiquettes pour 3j/7j où chaque pesée a de la place, 3 pour 14j/30j, 5 pour 90j), les indices étant choisis à intervalles réguliers dans le tableau de points (`evenIndices`).

**Date estimée d'atteinte** (`components/progression/GoalEstimateCard.tsx`, calcul dans `computeGoalEstimate`, `constants/progression.ts`) : régression linéaire (moindres carrés) sur les pesées des 14 derniers jours, extrapolée jusqu'à `profile.poids_objectif`. Trois états distincts plutôt qu'un seul texte optimiste : moins de 3 pesées sur cette fenêtre de 14 jours → invite à se peser plus régulièrement ; tendance quasi nulle (< 0,005 kg/jour) ou qui s'éloigne de l'objectif (le signe de la pente ne correspond pas au signe de l'écart restant) → "Continue tes efforts pour voir une estimation" plutôt qu'une date absurde ou dans le passé ; sinon, la date est affichée en toutes lettres (`toLocaleDateString('fr-FR', { day, month: 'long', year })`, ex. "14 novembre 2026") avec le nombre de jours restants en dessous.
...
**Moyenne calorique, énergie hebdomadaire et dépenses énergétiques.** `hooks/useMealStats.ts` (sur le modèle de `useWaterLogs`) lit `meals` sur 30 jours et zéro-remplit un total kcal par jour (`history`), d'où dérivent la moyenne 7 jours, la moyenne 30 jours et le total du jour — un jour sans repas enregistré compte pour 0 dans les moyennes plutôt que d'être exclu, même logique déjà en place pour la moyenne hebdomadaire d'hydratation. `components/progression/CalorieAverageCard.tsx` affiche ces deux moyennes à côté de l'objectif calorique du jour (`computeCalorieTarget`, `constants/dashboard.ts`) en trois colonnes. `components/progression/WeeklyEnergyCard.tsx` affiche sept barres — une par jour de la semaine calendaire en cours (`getCurrentWeekDays`, `constants/dashboard.ts`, donc toujours lundi à dimanche, pas les 7 derniers jours glissants), la barre du jour en `colors.accent` et les autres en `colors.border`, avec une ligne pointillée (`borderStyle: 'dashed'`) au niveau de l'objectif calorique et l'initiale du jour sous chaque barre. `constants/energy.ts` centralise la formule de Mifflin-St Jeor (`computeBmr`) et le multiplicateur d'activité (`computeTdee`) — `computeCalorieTarget` (`constants/dashboard.ts`) les réutilise pour son propre calcul plutôt que de dupliquer la formule, l'ajustement objectif/vitesse restant sa seule logique propre. `components/progression/EnergyExpenditureCard.tsx` affiche le métabolisme de base et la dépense totale estimée bruts (sans l'ajustement objectif/vitesse, contrairement à l'objectif calorique du dashboard), puis le déficit ou le surplus du jour face aux calories réellement consommées : la couleur du solde dépend du sens souhaité par l'objectif du profil — un déficit est "bon" (`colors.accent`) pour "Perte de poids" et "mauvais" (`colors.warning`) sinon, et inversement pour un surplus.

**Badges obtenus.** `components/progression/BadgesSummaryCard.tsx` réutilise `hooks/useBadges.ts` (déjà utilisé par le dashboard et `app/badges.tsx`) pour afficher la bande des badges débloqués (`BadgeMedal`, cercles 48px) avec le compteur "X / 8" ; toute la carte est un lien vers `/badges`. La section est entièrement masquée tant qu'aucun badge n'est débloqué, plutôt que d'afficher une carte vide.
...
**`lib/recipes.ts`** suit exactement le même pattern `fetch` brut que `lib/foodScanner.ts`/`lib/progressAnalysis.ts` (toujours pas de SDK Anthropic — voir la section Coach IA) : `generateCategoryRecipes(categoryPromptLabel, profile)` et `analyzeFridge(images, profile)` injectent objectif/poids actuel/poids cible/niveau d'activité/restrictions dans le `system` prompt (même convention conditionnelle que `buildSystemPrompt` dans `progressAnalysis.ts` — une ligne par champ non nul, et volontairement condensée pour limiter les tokens d'entrée). `analyzeFridge` construit un tableau `content` avec un bloc `image` par photo (jusqu'à 3) suivi d'un bloc `text` d'instructions, sur le modèle de `analyzeProgress` dans `progressAnalysis.ts`. Les deux renvoient un JSON strict (`stripJsonFences` avant `JSON.parse`, dans un try/catch, erreurs traduites via `describeAnthropicError`). Le prompt précise explicitement que les recettes doivent respecter toutes les restrictions alimentaires du profil sans exception et que chaque étape de préparation doit être détaillée et exploitable (2 à 3 phrases, températures, durées, techniques) plutôt qu'un simple résumé.

**Onglet Suggestions — navigation par catégories.** Un bandeau façon stories Instagram (`components/recipes/CategoryStoryBar.tsx`, bulles rondes 64px, bordure `colors.accent` 2px épaissie à 3px + fond teinté `hexToRgba(colors.accent, 0.12)` quand sélectionnée) propose 6 catégories (`constants/recipes.ts` — Petit-déj/Déjeuner/Dîner/Collation/Dessert/Végé, une icône Lucide chacune). Au montage, `getDefaultRecipeCategory()` présélectionne la catégorie selon l'heure courante (avant 10h30 → petit-déj, avant 14h30 → déjeuner, avant 17h30 → collation, sinon dîner). Changer de bulle appelle `generateCategoryRecipes` pour les `RECIPES_PER_CATEGORY` (6) recettes adaptées à la fois au profil et à la catégorie choisie ; le résultat du jour est mis en cache dans `AsyncStorage` par combinaison jour + catégorie (clé `recipes_category_{userId}_{date}_{categorie}`) — une réouverture de l'écran ou un retour sur une catégorie déjà consultée relit le cache (ou, dans la même session, directement le state en mémoire) plutôt que de rappeler l'IA. Le bouton "Générer d'autres idées" ignore toujours le cache, réappelle l'IA pour la catégorie affichée et écrase l'entrée du jour avec le nouveau résultat — sans toucher au cache ni aux favoris des autres catégories (`suggestionFavorites` est indexé par `"{categorie}:{index}"`, pas par un simple index, pour que deux catégories différentes ne collisionnent jamais). Sous le bandeau, un titre "Idées {catégorie}" et, si le profil a un objectif renseigné, "Adapté à ton objectif {objectif}". Pendant la génération, trois `RecipeSkeletonCard` (fond `colors.surface` pulsant via `Animated.loop`, même pattern que `TypingIndicator.tsx`) tant qu'aucun lot n'est revenu ; dès qu'un lot arrive ses recettes s'affichent immédiatement, avec un `RecipeSkeletonCard` de fin de liste tant qu'un second lot reste en cours. En cas d'échec (après le retry réseau interne), le message d'erreur et un bouton "Réessayer" s'affichent sous les recettes déjà obtenues plutôt qu'à leur place.

**Fiabilité et performance des appels IA.** `postAnthropicMessage` (interne à `lib/recipes.ts`) enveloppe chaque appel Anthropic avec un `AbortController` armé à 45s (au-delà, `recipes.errors.timeout` — "La génération prend trop de temps. Réessaie.") et une tentative automatique en cas d'échec réseau (pas de retry après un timeout, ça doublerait l'attente) ; chaque appel logue sa durée via `console.log` pour repérer ce qui traîne. La génération par catégorie ne demande plus 10 recettes en un seul appel (source des 5-10 minutes d'attente et des échecs fréquents observés) mais `RECIPES_PER_CATEGORY` (6) réparties en `CATEGORY_BATCH_COUNT` (2) appels de 3 recettes tirés en parallèle, chacun avec un `max_tokens` réduit au strict nécessaire (`CATEGORY_BATCH_MAX_TOKENS`) — `generateCategoryRecipes` accepte un callback `onBatch` optionnel appelé dès qu'un lot revient, ce qui permet à `app/recipes.tsx` d'afficher les recettes au fur et à mesure plutôt que d'attendre les deux lots (un lot en échec après retry laisse les recettes déjà affichées à l'écran plutôt que de tout effacer).

**`categorie_visuelle` et les photos de recette.** Le prompt de catégorie demande à l'IA de classifier chaque recette dans l'une de 8 catégories visuelles (`petit-dejeuner`, `salade`, `viande`, `poisson`, `pates`, `vegetarien`, `dessert`, `snack` — un vocabulaire volontairement différent des 6 catégories du bandeau, puisqu'une idée "Déjeuner" peut être visuellement une salade, un plat de viande, de poisson, de pâtes ou un plat végétarien) avec des exemples explicites (escalope de dinde à la moutarde → viande, saumon grillé → poisson, salade de quinoa → salade) pour éviter qu'elle ne classe le plat d'après son accompagnement. Le choix de l'IA n'est pas définitif : `matchVisualCategoryFromTitle(titre)` (`constants/images.ts`) reclasse la recette d'après des mots-clés dans son titre (poulet/dinde/bœuf/porc/veau/escalope → viande, saumon/cabillaud/thon/poisson/crevette → poisson, pâtes/spaghetti/lasagne/risotto → pates) et prime systématiquement sur `categorie_visuelle` — appliqué juste après le parsing JSON dans `lib/recipes.ts`, avant que la recette n'atteigne l'écran. `recipeImage(categorie)` (`constants/images.ts`) résout la catégorie finale vers `recipe-{categorie}.jpg` dans le bucket `app-images` (même convention que `appImage()`, voir la section Visuels), avec repli sur `recipe-vegetarien.jpg` si la valeur est absente ou non reconnue (recette du frigo, recette relue depuis les favoris — aucune des deux ne porte ce champ) ; **8 fichiers `recipe-{categorie}.jpg` restent à téléverser dans ce bucket** pour que les images s'affichent réellement. Ce champ est optionnel sur le type `Recipe` : seule la génération par catégorie le renvoie, `saved_recipes` ne le persiste pas (les favoris et Mon frigo gardent l'ancien rendu sans image, via `RecipeCard.tsx`).
...
**Cartes de recette.** `components/recipes/RecipeIdeaCard.tsx` (Suggestions uniquement) : bandeau image 120px (`AppImage`, voile 0.25, coins hauts arrondis via le `overflow: hidden` de la carte), cœur en superposition dans un cercle noir semi-transparent fixe de 32px (pas theme-réactif : il repose sur une photo, pas sur le fond de l'app — même logique que le voile d'`AppImage`), puis titre 16px gras, description 12px `colors.textSecondary` sur 2 lignes, rangée de 3 métadonnées (calories/temps/difficulté, icônes 12px `colors.accent`). `components/recipes/RecipeCard.tsx` (Mon frigo et Favoris, sans image) reste : fond `colors.surface`, bordure `colors.border`, rayon 16px, padding 16px, icône cœur en haut à droite — rendue comme un `Pressable` frère du corps de la carte (pas imbriqué dans le `Pressable` principal, pour éviter tout conflit de capture tactile entre les deux zones tapables).
...
**Tables Supabase** (voir la migration complète donnée dans la conversation qui a introduit cette fonctionnalité) : `groups` (avec `avatar_path`, `banner_path`, `is_private`), `group_members` (unique sur `(group_id, user_id)`, `role` `admin`/`membre`, `last_read_at` pour les compteurs de non-lus), `group_messages` (suppression douce via `deleted_at`, `reply_to_id` pour les réponses citées), `message_reactions` (unique sur `(message_id, user_id, emoji)`), `message_reports`, `blocked_users`, `group_join_requests` (`status` `en_attente`/`accepte`/`refuse`, unique sur `(group_id, user_id)`). RLS sur les sept tables, avec une fonction `is_group_member(gid uuid) security definer` pour éviter la récursion des policies de `group_members` (une policy de lecture sur `group_members` qui interrogerait elle-même `group_members` retriggerait indéfiniment la même policy) et `is_group_admin(gid uuid) security definer` sur le même principe pour les policies de `group_join_requests` et l'`update` de `groups`. Bucket privé `group-images` (objets préfixés `{group_id}/...`, `avatar.jpg`/`banner.jpg` en plus des photos de message), policies utilisant la même fonction `is_group_member`.

**Rejoindre un groupe privé** ne peut pas se faire par un simple insert client (accepter une demande insère un membre pour un `user_id` différent de l'appelant, hors de portée d'une policy RLS "chacun insère sa propre ligne"). Deux fonctions `security definer`, `accept_group_join_request(request_id)` et `refuse_group_join_request(request_id)`, vérifient que l'appelant est admin du groupe de la demande puis font la mise à jour + l'insertion dans `group_members` (côté accepté) dans une seule transaction — appelées depuis `app/group/[id]/info.tsx` via `supabase.rpc(...)`.

**`hooks/useGroups.ts`** — liste des groupes de l'utilisateur (rôle, nombre de membres, aperçu du dernier message, avatar signé en lot via `signGroupImagePaths` (`lib/groups.ts`), compteur de non-lus calculé côté client depuis `last_read_at`), `joinByCode` (vérifie l'existence du code et la non-appartenance avant d'insérer ; si le groupe est privé, crée une ligne `group_join_requests` — `en_attente` existante retournée comme "déjà en attente", `refuse` existante bloque une nouvelle demande via la contrainte d'unicité et remonte une erreur dédiée plutôt qu'un échec générique — au lieu d'ajouter directement le membre), `createGroup` (génère un code à 6 caractères alphanumériques uniques via `lib/groups.ts`, insère le créateur comme `admin`).
...
**`vercel.json`** réécrit toute route vers `/index.html` — sans ça, rafraîchir sur une route interne (`/coach`, `/skincare`...) renvoie une 404 puisque ces chemins n'existent pas comme fichiers sur le serveur. Le `source` exclut explicitement par nom les fichiers statiques servis à la racine (`favicon.ico`, `favicon-32.png`, `favicon-16.png`, `apple-touch-icon.png`, `icon-192.png`, `icon-512.png`, `manifest.json`) via un lookahead négatif (`/((?!favicon\.ico|...).*)`) plutôt que de compter uniquement sur la priorité usuelle de Vercel pour les fichiers réels du dossier de sortie — plus explicite, et robuste face à d'éventuelles différences de comportement selon la configuration du projet Vercel (répertoire de sortie, versions de routing).

**Favicons (`public/favicon.ico`, `favicon-32.png`, `favicon-16.png`, `apple-touch-icon.png`)** — sans un `favicon.ico` fourni à la racine du site, Vercel sert son propre favicon par défaut à la place, quoi que fasse `public/index.html`. Les quatre fichiers sont générés depuis `assets/icon.png` (1254×1254, déjà opaque avec un fond quasi identique à `#0a0d0c`) via Pillow : `favicon.ico` multi-résolution (16/32/48px, un seul fichier `.ico` avec les trois tailles embarquées via `Image.save(..., format="ICO", sizes=[...])`), `favicon-32.png`/`favicon-16.png` en PNG simple, et `apple-touch-icon.png` à 180×180 aplati sans transparence sur fond `#0a0d0c` (`Image.new("RGB", ...)` + `paste` avec le canal alpha comme masque, même si la source n'a déjà aucune transparence — le code reste correct si la source venait à changer). Référencés dans `public/index.html` via des `<link rel="icon">`/`<link rel="apple-touch-icon">` explicites plutôt que de compter sur l'injection automatique d'Expo (qui n'aurait pris que `app.json`'s `web.favicon`, un seul fichier, sans les tailles 16/32 dédiées). `expo.web.favicon` (`app.json`) pointe malgré tout vers `./assets/icon.png` (plus l'ancien `./assets/favicon.png`, l'icône par défaut d'Expo) par cohérence, même si ce chemin n'est concrètement utilisé que par le template HTML par défaut d'Expo — remplacé ici par `public/index.html` (voir plus bas).
...
`npx expo export --platform web` a été vérifié après coup : `index.html` exporté contient bien le titre, la description, les balises Open Graph, le lien manifeste et les balises iOS, et `manifest.json`/`icon-192.png`/`icon-512.png`/`favicon.ico`/`favicon-32.png`/`favicon-16.png`/`apple-touch-icon.png` sont bien présents à la racine de la sortie (`dist/`).
...
- **Onglet Séance** et **détail de séance** (`app/workout/[id].tsx`) : même bandeau `plan-hero.jpg` que l'écran Plan (140px) en tête de l'onglet, et chaque exercice listé reçoit une vignette 48px via `exerciseImage(nom)` (`constants/images.ts` — classification par mots-clés dans le nom de l'exercice : squat/jambe/fente → `exercise-squat.jpg`, course/cardio/corde → `exercise-cardio.jpg`, pompe/gainage/abdo → `exercise-squat.jpg` en attendant l'upload de `exercise-pushup.jpg` dans le bucket, tout le reste → `exercise-dumbbells.jpg`).
...
## Adaptation aux écrans larges (desktop/tablette)

L'app reste mobile-first : chaque comportement desktop est additif et conditionné par `useBreakpoint()` (`hooks/useBreakpoint.ts`, basé sur `useWindowDimensions` — réactif au redimensionnement web, contrairement à `Dimensions.get`) — `mobile` en dessous de 768px, `tablet` de 768 à 1023px, `desktop` à partir de 1024px. En dessous de 768px, aucun fichier touché par ce chantier n'a de branche de rendu différente : le JSX mobile est resté strictement identique à ce qu'il était avant, seule une branche `isDesktop` a été ajoutée à côté.

**`components/ui/ResponsiveContainer.tsx`** plafonne la largeur du contenu à 1200px sur desktop (centré, 32px de marge de chaque côté) ; en dessous du seuil desktop, c'est un simple passe-plat (`<View style={style}>{children}</View>`) qui ne change rien au rendu. Le `style` passé est appliqué à la boîte de contenu elle-même (pas au conteneur englobant), pour que les appelants puissent y ajouter leur propre `gap`/`flex` sans casser le plafonnement de largeur.

**Barre latérale desktop (`components/ui/DesktopSidebar.tsx`)** remplace la barre d'onglets flottante à partir de 1024px : 240px de large, fond `colors.surface`, bordure droite `colors.border`, logo en tête, puis les 11 destinations (Accueil, Coach, Scanner, Progression, Séances, Repas, Recettes, Groupes, Badges, Skincare, Profil — le bouton **+** n'est donc plus nécessaire sur desktop, tout est déjà visible). Elle est montée une seule fois dans `app/_layout.tsx` (pas dans `(tabs)/_layout.tsx`) pour rester visible même sur les écrans empilés en dehors du groupe `(tabs)` (badges, recettes, groupes, skincare...) ; `(tabs)/_layout.tsx` masque en retour sa barre flottante et son bouton **+** (`tabBarStyle: { display: 'none' }`) uniquement quand `isDesktop` est vrai.

**Grilles adaptatives** via `components/ui/ResponsiveGrid.tsx` (pourcentage de largeur par carte selon le nombre de colonnes demandé, `flexWrap` sur desktop, pile simple sinon) : Recettes passe à 3 colonnes, Séances recommandées (dashboard) à 2. Badges (déjà organisé en lignes chunkées manuellement, `chunkRows`) passe de 2 à 4 par ligne en changeant simplement la taille du chunk. Groupes utilise directement `numColumns` de `FlatList` (2 sur desktop, `key={numColumns}` pour forcer un remount propre au changement de colonnes, `FlatList` refusant sinon un changement de `numColumns` à chaud). Le dashboard lui-même passe en deux colonnes (cartes principales à gauche, séances/BMI/stats/badges/astuce à droite) — le JSX mobile et desktop partagent les deux mêmes fragments (`mainColumn`/`secondaryColumn`), seule leur disposition (empilés vs. côte à côte) change.

**Coach et conversation de groupe** : les bulles de message (`components/coach/MessageBubble.tsx`, `components/groups/MessageBubble.tsx`) passent de 80%/78% à 60% de largeur max sur desktop. `app/group/[id].tsx` affiche en plus un panneau gauche de 320px (`DesktopGroupsListPane`, réutilise `useGroups`/`GroupCard`) listant les autres groupes de l'utilisateur, avec `router.replace` pour changer de conversation sans empiler d'écran. Le coach n'a qu'une seule conversation (pas de liste à afficher) : son traitement desktop se limite à des bulles resserrées et un contenu centré via `ResponsiveContainer`.

**Modals** : `constants/modalPresentation.ts` exporte deux objets de style partagés (`desktopModalBackdrop`, `desktopModalSheet`) appliqués à la quasi-totalité de l'inventaire des modales (feuilles glissant du bas) — centré au lieu de collé en bas, coins arrondis sur les 4 côtés au lieu des 2 du haut, largeur plafonnée à 480px. Chaque modale garde en plus son fond cliquable existant pour se fermer (aucun nouveau code nécessaire, déjà présent partout) et gagne un raccourci clavier Échap via `hooks/useEscapeKey.ts` (no-op sur natif — écoute `document.keydown`, uniquement montée si `Platform.OS === 'web'`). `BadgeUnlockModal`/`ImageViewerModal`, déjà centrées nativement, ne reçoivent que le raccourci Échap.

**Onboarding** : `constants/onboardingLayout.ts` expose `ONBOARDING_MAX_WIDTH` (560px) et un style prêt à l'emploi, appliqué à chaque écran du flow (`welcome`, `analyse`, `plan`, `signup`, `login`, `forgot-password`, `paywall`) ainsi qu'à `components/onboarding/OnboardingLayout.tsx` (partagé par tous les écrans de question `q/[step]`, cartes d'option comprises) — un seul point d'edit couvre donc tout le questionnaire.

**Raccourcis clavier web** (`lib/webKeyboard.ts`) : `createEnterToSendHandler` branché sur `onKeyPress` des champs de saisie coach/groupes — Entrée envoie, Maj+Entrée insère un retour à la ligne. Fonctionne car `react-native-web` transmet l'événement clavier DOM réel (`nativeEvent.key`/`shiftKey`) même si les types RN ne déclarent que `key` ; sur natif l'événement ne se déclenche simplement jamais avec `key: 'Enter'` dans ce contexte, donc c'est un no-op silencieux.

**Survol web** (`hooks/useHover.ts`, via `onHoverIn`/`onHoverOut` — supportés par le type `Pressable` de React Native mais qui ne se déclenchent qu'à la souris, donc de facto web-only) : branché sur la barre latérale desktop (fond `#1a1f18` au survol) et sur `components/ui/Button.tsx` (opacité 0.85), qui couvre la plupart des actions principales de l'app. Le curseur `pointer` sur web n'a nécessité aucun code : `react-native-web` l'applique déjà par défaut à tout `Pressable` non désactivé. Le survol n'a pas été étendu à chaque `Pressable` custom de l'app (des dizaines d'écrans/composants) — à généraliser à d'autres composants réutilisés au besoin.

**Reste hors de ce chantier** : les écrans profil, progression, séances, repas, scanner, skincare, journal, jeûne et les vues de détail (recette, séance, info de groupe) n'ont pas encore reçu `ResponsiveContainer` — ils restent utilisables sur desktop (rendu mobile inchangé, juste étiré pleine largeur) mais pas encore recentrés/plafonnés.
