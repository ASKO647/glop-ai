...
La barre n'affiche que 4 onglets — Accueil, Coach, Scanner, Profil — plus un bouton **+** qui n'est pas un onglet : `progression`, `workout` et `meals` sont déclarés comme `Tabs.Screen` avec `href: null`, ce qui les retire de la barre sans les retirer du routeur (`/progression`, `/workout`, `/meals` restent navigables en poussant l'écran par-dessus les tabs). Contrairement à la première version, le bouton **+** ne chevauche plus la pilule : c'est un cercle séparé, posé à sa droite avec un écart de 12px (`FAB_GAP`), de diamètre exactement égal à la hauteur de la barre (`FAB_SIZE = TAB_BAR_HEIGHT`) — les deux partagent donc le même sommet et la même ligne de base, sans qu'aucun ne dépasse l'autre. Concrètement, la pilule (`tabBarStyle`) et le bouton (`Pressable` à part, positionné en `absolute`) sont deux éléments frères positionnés indépendamment à partir des mêmes bords d'écran plutôt qu'un vrai flex row : la pilule garde `left: TAB_BAR_MARGIN` mais son `right` est poussé à `TAB_BAR_MARGIN + FAB_SIZE + FAB_GAP` pour laisser la place au bouton, et le bouton garde `right: TAB_BAR_MARGIN` — React Navigation rend son `<BottomTabBar>` en dehors du flux JSX qu'on contrôle, donc un vrai `flexDirection: 'row'` n'est pas possible ici. Les deux éléments partagent le même objet de style d'ombre (`BAR_SHADOW`) pour rester visuellement cohérents, et le même calcul de `bottom` (`Math.max(24, insets.bottom + 12)`), donc alignés au pixel près. Avec seulement 4 libellés courts restants dans la barre (le plus long est "Scanner"), aucun n'est tronqué même sur la pilule rétrécie par l'espace réservé au bouton.
